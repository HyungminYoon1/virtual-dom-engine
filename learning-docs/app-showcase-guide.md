...
추가로 현재 구현은 성능을 위해 Collection을 가상 스크롤 방식으로 렌더한다.

- 전체 1025장을 한 번에 DOM에 올리지 않는다.
- 현재 화면 근처 카드만 렌더한다.
- 스크롤 높이는 전체 목록이 있는 것처럼 유지한다.
- 데스크톱에서는 카드가 최소 3열 이상 보이도록 본문 폭을 우선 배분한다.
...
- 초기 로드: 먼저 `#001 ~ #1025` shell 카드를 고정 순서로 만들고, 이후 현재 보이는 구간의 카드 정보만 점진 hydrate
...
- 설정 반영
- 원격 카탈로그 fallback 안내
- Inspector가 `img src` patch를 보여주는지
- Collection이 대량 카탈로그에서도 현재 구간만 렌더하는지
...
6. Inspector는 데스크톱에서만 보이고, 모바일에서는 콘텐츠 우선으로 숨긴다.
...
현재 앱은 초기 기획에서 더 발전한 형태로 아래 동작을 포함한다.

- Collection 페이지는 가상 스크롤 캔버스를 사용해 현재 화면 근처 카드만 DOM에 올린다.
- 원격 데이터는 먼저 `#001 ~ #1025` shell 카드를 고정 순서로 준비하고, 이후 현재 화면 근처 카드 정보만 hydrate 한다.
- 타입 필터를 사용할 때는 전체 타입 데이터를 먼저 로드한 뒤 결과를 표시한다.
- UI locale 과 포켓몬 표시 이름 locale 은 함께 동작하며, 지원 언어는 English, 한국어, 日本語, 中文, Español 이다.
- 포켓몬 이름은 로컬 이름 사전을 우선 사용하고, 사전에 없는 항목만 pokemon-species 를 예외 fallback 으로 조회한다.
- Collection 타입 배지는 18개 포켓몬 타입 전체에 대해 공식 타입 팔레트 톤을 사용한다.
- Collection 가상 스크롤은 함수형 자식 key 보존과 patch 순서 보정으로 카드 재사용 시 타입 배지가 뒤섞이지 않도록 안정화돼 있다.
- Collection 스크롤 컨테이너는 유지하고 내부 window만 교체해 스크롤 위치를 보존한다.
- Detail 페이지의 연관 포켓몬은 같은 진화트리를 먼저 표시하고, 부족한 칸은 같은 타입과 유사한 종족값을 가진 카드로 채운다.
- Detail 페이지에는 메인 카드 외에 Game Sprite 비교 카드가 함께 표시된다.
- Render / Patch Inspector 는 데스크톱에서만 시연용으로 보이며, 모바일에서는 숨긴다.
