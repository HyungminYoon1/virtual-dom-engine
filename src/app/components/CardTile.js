...
export function CardTile(props) {
  ...
  return h("article", {
    ...
    h("button", {
      ...
      onClick: props.onOpenClick ?? (() => props.onSelect(card.id)),
    },
      ...
      h("div", { className: "card-overlay" },
        ...
        h("strong", { className: "card-name" }, displayName),
        h("div", { className: "card-type-row" }, ...renderTypeBadges(card.types, props.typeLabels))
      )
    ),
    h("div", { className: "card-meta-row" },
      ...
      h("button", {
        ...
        "data-card-id": card.id,
        onClick: props.onFavoriteClick ?? (() => props.onToggleFavorite(card.id)),
      }, card.isFavorite ? props.copy.common.saved : props.copy.common.save)
    )
  );
}
